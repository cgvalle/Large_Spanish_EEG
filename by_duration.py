...
if not os.path.exists(f"results/duration/{args.name}/stims_{args.stims}"):
    os.makedirs(f"results/duration/{args.name}/stims_{args.stims}")


for window in [ 50, 125,  250,  375,  500,  625,  750,  875, 1000]:
    P = defaultdict(list)
    for iteration in tqdm(range(args.num_runs)):
        ...
        model = EEGNet(len(STIMS), Chans = 14, Samples = window, 
        ...
        np.save(f'results/duration/{args.name}/stims_{args.stims}/{window}_task_{args.task}_shuffle_{args.shuffle}', P) 